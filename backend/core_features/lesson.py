...
get_lessons_by_rcourse_id.add_argument("rcourse_id", help="Enter rcourse id")
...
class GetLessonsByRcourseId(Resource):
    ...
    def get(self):
        ...
        def get_runcourse_details(rcourse_id):
            try: 
                run_course = RunCourse.query.filter_by(rcourse_ID=rcourse_id).first()
                if run_course:
                    run_course.run_Starttime = run_course.run_Starttime.strftime('%H:%M:%S')
                    run_course.run_Endtime = run_course.run_Endtime.strftime('%H:%M:%S')
                    return jsonify(
                        {
                            "code": 200,
                    ...
                    return jsonify(
                        {
                            "code": 400,
                            'message': "An error has occurred while retrieving run details",
                        }
                    )
            except Exception as e:
                return {"code": 404, "message": "Failed " + str(e)}, 404
        ...
        try:
            rcourseID = get_lessons_by_rcourse_id.parse_args().get("rcourse_id")
            course_lessons = Lesson.query.filter_by(rcourse_ID=rcourseID).all()
            lessons = []
            if course_lessons:
                for lesson in course_lessons:
                    rcourse_id = lesson.rcourse_ID
                    runcourse_response = get_runcourse_details(rcourse_id)
                    if runcourse_response.code == 200:
                        lessons.append({
                            "lesson_ID": lesson.lesson_ID,
                            "rcourse_ID": lesson.rcourse_ID,
                            "lesson_Date": lesson.lesson_Date,
                            "run_Name": runcourse_response.run_course.run_Name,
                            "lesson_Starttime": runcourse_response.run_course.run_Starttime,
                            "lesson_Endtime": runcourse_response.run_course.run_Endtime,
                            "run_course": runcourse_response.run_course
                        })
                    else: 
                        return  runcourse_response
                    print(lesson)
