        ...
        <table class="table bg-white">
          ...
          <tbody>
            <tr v-for="(course, key) in displayedCourses" :key="key">
              ...
              <td v-if="course.run_Startdate && isBeforeCurrentDate(course.run_Startdate)"><a v-if="course.course_Status != 'Retired'" class="btn btn-info text-light font-weight-bold" @click="openFeedbackTemplateModal(course)" data-bs-toggle="modal" data-bs-target="#apply_course_feedback_template_modal">Apply Feedback Template</a></td>
              <td v-else><a v-if="course.course_Status != 'Retired'" class="btn btn-info disabled text-light font-weight-bold" title="Unable to remove run course due to ongoing/past feedback period'">Apply Feedback Template</a></td>
